...
class ServiceOrderOperationsTest(unittest.TestCase):
    ...
    def test_intake_number_triage_tasks_timeline_and_duplicate(self):
        ...
        detail=self.endpoint("/api/v1/service-orders/{order_id}","GET")(order["id"],self.user,self.db);self.assertEqual(len(detail["tasks"]),1);self.assertEqual(len(detail["timeline"]),4)
