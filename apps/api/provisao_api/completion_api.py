...
from .models import (ServiceOrder, ServiceOrderDelivery, ServiceOrderEvent, ServiceOrderSequence, ServiceOrderWorkSession, ServiceVisit, User, Warranty, WarrantyReturn)
...
def build_router(current_user:Callable,require:Callable,audit:Callable)->APIRouter:
    ...
    def warranty(order_id:str,payload:WarrantyPayload,user:User=Depends(require("warranty.manage")),session:Session=Depends(db)):
        ...
        item=Warranty(company_id=user.company_id,service_order_id=order.id,created_by=user.id,**payload.model_dump());session.add(item);session.flush();session.add(ServiceOrderEvent(service_order_id=order.id,actor_id=user.id,event_type="warranty_created",detail="warranty registered"));audit(session,user,"warranty_created","warranty",item.id);session.commit();return {"id":item.id,"service_order_id":item.service_order_id,"starts_at":item.starts_at,"ends_at":item.ends_at,"status":item.status}
