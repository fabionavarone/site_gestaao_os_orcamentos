...
from .models import Attachment, AuditLog, Branch, Channel, ChannelBot, Company, Conversation, ConversationMessage, Customer, Equipment, ExternalEvent, OutboxEvent, Role, ServiceOrder, ServiceOrderEvent, ServiceOrderSequence, Session as UserSession, Team, TeamMember, User, UserRole, WorkflowInstance, WorkflowState, WorkflowTransition
...
from .services.workflow import WorkflowError, ensure_default_workflow, execute_transition, start_instance
...
def create_order(payload: OrderIn, user: User=Depends(require("service_order.create")), session: Session=Depends(db)):
    ...
    if not sequence:
        next_number=(session.scalar(select(func.max(ServiceOrder.number)).where(ServiceOrder.company_id==user.company_id)) or 0)+1
        sequence=ServiceOrderSequence(company_id=user.company_id,next_number=next_number);session.add(sequence);session.flush()
    number=sequence.next_number;sequence.next_number+=1
    item=ServiceOrder(company_id=user.company_id,number=number,**payload.model_dump()); session.add(item); session.flush()
    workflow_version=ensure_default_workflow(session,user.company_id,user.id);instance=start_instance(session,company_id=user.company_id,workflow_version=workflow_version,entity_type="service_order",entity_id=item.id,actor_id=user.id);item.workflow_instance_id=instance.id
    session.add(ServiceOrderEvent(service_order_id=item.id,actor_id=user.id,event_type="created",detail="service order created with published workflow")); audit(session,user,"create","service_order",item.id,{"number":number,"workflow_instance_id":instance.id}); session.commit(); return {"id":item.id,"number":number,"version":item.version,"workflow_instance_id":instance.id}
@app.post("/api/v1/service-orders/{order_id}/transition")
def transition_order(order_id: str,payload: TransitionIn,user: User=Depends(require("service_order.transition")),session: Session=Depends(db)):
    ...
    instance=session.scalar(select(WorkflowInstance).where(WorkflowInstance.id==item.workflow_instance_id,WorkflowInstance.company_id==user.company_id))
    if not instance: raise HTTPException(409,"service order has no workflow instance")
    destination=session.scalar(select(WorkflowState).where(WorkflowState.workflow_version_id==instance.workflow_version_id,WorkflowState.code==payload.status))
    transition=session.scalar(select(WorkflowTransition).where(WorkflowTransition.workflow_version_id==instance.workflow_version_id,WorkflowTransition.from_state_id==instance.current_state_id,WorkflowTransition.to_state_id==destination.id if destination else False,WorkflowTransition.active.is_(True)))
    if not transition: raise HTTPException(409,"invalid service order transition")
    old=item.status
    try: execute_transition(session,instance=instance,transition=transition,actor_id=user.id,permissions={"service_order.transition"},reason=payload.reason,context={"assigned":False,"checklist_complete":False,"diagnosis_approved":False},handlers={})
    except WorkflowError as exc: raise HTTPException(422,str(exc)) from exc
    item.status=payload.status;item.version+=1;session.add(ServiceOrderEvent(service_order_id=item.id,actor_id=user.id,event_type="status_changed",detail=f"{old} -> {item.status}: {payload.reason or ''}"));audit(session,user,"transition","service_order",item.id,{"from":old,"to":item.status,"workflow_instance_id":instance.id});session.commit();return {"id":item.id,"status":item.status,"version":item.version,"workflow_instance_id":instance.id}
