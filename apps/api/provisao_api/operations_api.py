...
from .models import (Channel, Conversation, Customer, Equipment, ServiceOrder,
                     ServiceOrderChecklist, ServiceOrderDiagnosis, ServiceOrderEvent,
                     ServiceOrderMeasurement, ServiceOrderSequence, ServiceOrderTask,
                     ServiceOrderTechnicalTest, ServiceOrderTriage, Team, User,
                     WorkflowInstance)
...
def build_router(current_user:Callable,require:Callable,audit:Callable)->APIRouter:
    ...
    def detail(order_id:str,user:User=Depends(current_user),session:Session=Depends(db)):
        item=owned(session,user,order_id); tasks=session.scalars(select(ServiceOrderTask).where(ServiceOrderTask.service_order_id==item.id).order_by(ServiceOrderTask.created_at)).all(); events=session.scalars(select(ServiceOrderEvent).where(ServiceOrderEvent.service_order_id==item.id).order_by(ServiceOrderEvent.created_at)).all(); triage=session.scalar(select(ServiceOrderTriage).where(ServiceOrderTriage.service_order_id==item.id)); diagnoses=session.scalars(select(ServiceOrderDiagnosis).where(ServiceOrderDiagnosis.service_order_id==item.id).order_by(ServiceOrderDiagnosis.version.desc())).all(); measurements=session.scalars(select(ServiceOrderMeasurement).where(ServiceOrderMeasurement.service_order_id==item.id).order_by(ServiceOrderMeasurement.created_at.desc())).all(); technical_tests=session.scalars(select(ServiceOrderTechnicalTest).where(ServiceOrderTechnicalTest.service_order_id==item.id).order_by(ServiceOrderTechnicalTest.created_at.desc())).all(); checklists=session.scalars(select(ServiceOrderChecklist).where(ServiceOrderChecklist.service_order_id==item.id)).all(); return {"id":item.id,"number":item.number,"customer_id":item.customer_id,"equipment_id":item.equipment_id,"conversation_id":item.conversation_id,"title":item.title,"symptom":item.symptom,"status":item.status,"priority":item.priority,"service_type":item.service_type,"team_id":item.team_id,"responsible_id":item.responsible_id,"technician_id":item.technician_id,"sla_due_at":item.sla_due_at,"sla_violated":sla_violated(item.sla_due_at,item.status),"tasks":[task_view(x) for x in tasks],"triage":triage_view(triage) if triage else None,"diagnoses":[diagnosis_view(x) for x in diagnoses],"measurements":[measurement_view(x) for x in measurements],"technical_tests":[technical_test_view(x) for x in technical_tests],"checklists":[checklist_view(x) for x in checklists],"timeline":[{"id":x.id,"event_type":x.event_type,"detail":x.detail,"actor_id":x.actor_id,"created_at":x.created_at} for x in events]}
...
def diagnosis_view(x): return {"id":x.id,"version":x.version,"summary":x.summary,"details":x.details,"risk":x.risk,"repairable":x.repairable,"recommendation":x.recommendation,"status":x.status,"technician_id":x.technician_id,"reviewer_id":x.reviewer_id,"reviewed_at":x.reviewed_at}
def measurement_view(x): return {"id":x.id,"parameter":x.parameter,"value":x.value,"unit":x.unit,"instrument":x.instrument,"condition":x.condition,"result":x.result,"performed_by":x.performed_by,"created_at":x.created_at}
def technical_test_view(x): return {"id":x.id,"name":x.name,"procedure":x.procedure,"result":x.result,"status":x.status,"observation":x.observation,"performed_by":x.performed_by,"created_at":x.created_at}
def checklist_view(x): return {"id":x.id,"template_id":x.template_id,"template_version":x.template_version,"items":x.items or [],"status":x.status,"completed_by":x.completed_by,"completed_at":x.completed_at}
