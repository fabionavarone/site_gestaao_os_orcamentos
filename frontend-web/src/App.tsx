...
function ServiceOrders() {
  const [query, setQuery] = useState(''); const [selected, setSelected] = useState<string>(); const [status, setStatus] = useState('');
...
export default function App() {
  const [user, setUser] = useState<User>(); const [checking, setChecking] = useState(true); const [tab, setTab] = useState<'inbox' | 'bots' | 'deliveries' | 'workflows' | 'customers' | 'equipment' | 'orders'>('inbox');
  ...
  return <div className="min-h-screen"><header className="border-b bg-white"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 p-4"><div><strong>Provisão Manager</strong><span className="ml-2 muted">{user.name}</span></div><nav aria-label="Principal" className="flex flex-wrap gap-2"><button onClick={() => setTab('inbox')}>Inbox</button><button onClick={() => setTab('customers')}>Clientes</button><button onClick={() => setTab('equipment')}>Equipamentos</button><button onClick={() => setTab('orders')}>OS</button><button onClick={() => setTab('bots')}>Bots</button><button onClick={() => setTab('deliveries')}>Entregas</button><button onClick={() => setTab('workflows')}>Workflows</button><button className="bg-slate-600" onClick={async () => { await api('/auth/logout', { method: 'POST' }); setUser(undefined); }}>Sair</button></nav></div></header><main className="mx-auto max-w-7xl p-4">{tab === 'inbox' ? <Inbox /> : tab === 'customers' ? <Customers /> : tab === 'equipment' ? <EquipmentPage /> : tab === 'orders' ? <ServiceOrders /> : tab === 'bots' ? <Bots /> : tab === 'deliveries' ? <Deliveries /> : <Workflows />}</main></div>;
}
